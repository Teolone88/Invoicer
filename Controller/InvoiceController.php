...
class InvoiceController extends FPDF
{
     ...
     ******************************************/
    public function __constructInvoice($size='A4',$currency='$',$language='en') {
        $this->columns  		  	= 4;
        $this->items 			  = array();
        $this->totals 			 = array();
        $this->addText 			= array();
        ...
        parent::__CONSTRUCTINVOICE('P','mm',array($this->document['w'],$this->document['h']));
        $this->AliasNbPages();
    ...
    public function Body() {
        ...
        if($this->items) {
            foreach($this->items as $item)
            {
                ...
                {
                    //Precalculate height
                    $calculateHeight = new InvoiceController;
